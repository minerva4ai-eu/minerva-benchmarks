...
# Usage: source run_single_benchmark.sh <CLUSTER> <PACKAGE_MANAGER> <FRAMEWORK> <MODEL> <DATASET> <NODES> <GPUS_PER_NODE> <PIPELINE_PARALLEL> <TENSOR_PARALLEL> <ITERATIONS> <CPUS_PER_TASK> <NTASKS_PER_NODE> 
# Example: bash run_single_benchmark.sh CINECA-Leonardo Conda deepspeed-MII Mistral-7B-Instruct-v0.3 sharegpt 1 1 1 1 1 1 1
...
if [ "$#" -ne 12 ]; then
    echo "Usage: source run_single_benchmark.sh <CLUSTER> <PACKAGE_MANAGER> <FRAMEWORK> <MODEL> <DATASET> <NODES> <GPUS_PER_NODE> <PIPELINE_PARALLEL> <TENSOR_PARALLEL> <ITERATIONS> <CPUS_PER_TASK> <NTASKS_PER_NODE>"
    exit 1
fi
