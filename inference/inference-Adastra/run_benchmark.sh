        ...
        for GPU_NODE in "${GPU_CONFIGS[@]}"; do
          for MAX_MODEL_LENGTH in "${MAX_MODEL_LENGTHS[@]}"; do
            ...
            if [[ "$framework" == "vllm" ]]; then
              ...
              if [[ "$model" == "meta-llama/Llama-3.1-405B" ]]; then
              ...
              for (( run_id=1; run_id<=REPEATS; run_id++ )); do
                ...
                if [ "${#JOB_IDS[@]}" -ge "$MAX_ACTIVE" ]; then
                  DEPENDENCY="--dependency=afterany:${JOB_IDS[$((CONFIG_INDEX - MAX_ACTIVE))]}"
                else
                  DEPENDENCY=""
                fi
