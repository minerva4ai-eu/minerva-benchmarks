...
head_node_ip=$(srun --nodes=1 --ntasks=1 -w "$head_node" hostname --short)

# Get the IP addresses for each node
for node in "${nodes_array[@]}"; do
    node_ip=$(srun --nodes=1 --ntasks=1 -w "$node" hostname --short)
...
srun --nodes=1 --ntasks-per-node=1 --cpu-bind=none --mem-bind=none -w "$head_node" --export=ALL,VLLM_HOST_IP=$head_node_ip bash run_cluster.sh $head_node_ip --head $MODEL_PATH $LAUNCH_FOLDER $BENCHMARK_FILE $DATASET $DATASET_PATH $ADDITIONAL_ARGS &
sleep 60
SRUN_PID=$!
...
for ((i = 1; i <= worker_num; i++)); do
    ...
    local_node_ip=$(srun -n 1 -N 1 -c 1 -w "$node_i" hostname --short)
    export VLLM_HOST_IP=$local_node_ip
    ip_local=$local_node_ip:$port
    srun --nodes=1 --ntasks-per-node=1 --cpu-bind=none --mem-bind=none -w "$node_i" --export=ALL,VLLM_HOST_IP=$local_node_ip bash run_cluster.sh $head_node_ip --worker $MODEL_PATH $LAUNCH_FOLDER $BENCHMARK_FILE $DATASET $DATASET_PATH $ADDITIONAL_ARGS &
    sleep 3
done
