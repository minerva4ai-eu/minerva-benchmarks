...
MODELS=("Llama-3.1-8B-Instruct") # "Llama-3.1-1B") # Llama-3.3-70B-Instruct") # "Llama-3.1-405B") # ("Llama-3.1-405B" "gemma-3-12b-it" "Mistral-7B-Instruct-v0.3") # Add your models here
NUMBER_OF_NODES=(1)
  ...
  for dataset in "${DATASETS[@]}"; do
    for model in "${MODELS[@]}"; do
      for NODES in "${NUMBER_OF_NODES[@]}"; do
        ...
        if [[ "$MACHINE_TYPE" -eq "cpu" ]]; then
            GPU_CONFIGS=(0)  # use default
        else
          if [[ "$NODES" -eq 1 ]]; then
