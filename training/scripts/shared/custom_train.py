...
class PerformanceTrackingSFTTrainer(SFTTrainer):
    ...
    def log(self, logs: Dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if self.logged_flops_this_gpus:
            ...
            logs[f"TFLOPs/sec/{device_type}"] = f"{self.logged_flops_this_gpus[-1] / 1e12:.2f}"
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        if self.step_interval_time > 0 and not os.environ.get("DISABLE_MONITORING", "False").lower() == "true":
    ...
    def train(self, *args, **kwargs):
        ...
        if self.is_world_process_zero():
            ...
            print(f"Total tokens per {device_type} (ALL epochs): {self.total_tokens_this_gpu:,}")
