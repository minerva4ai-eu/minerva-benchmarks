...
# Only set CUDA-specific configurations if CUDA is available
...
class PerformanceTrackingTrainer(Trainer):
    ...
    def train(self, *args, **kwargs):
        ...
        try:
            self.state.total_training_seconds_custom = float(elapsed)
            self.state.total_tokens_per_gpu_custom = int(self.total_tokens_this_gpu)
            self.state.total_tokens_global_custom = int(self.total_tokens_global)
            self.state.average_flops_custom = float(self.avg_flops_this_gpu)
            self.state.average_mfu_custom = float(self.avg_mfu_this_gpu)

            if "__get_item__" in self.train_dataset.__dict__:
                if self.train_dataset.__dict__["__get_item__"]:
                    self.state.average_get_item_time = sum(
            ...
            if "collate_fn" in self.train_dataset.__dict__:
                if self.train_dataset.__dict__["collate_fn"]:
                    self.state.average_collate_fn_time = sum(
...
class PerformanceTrackingSFTTrainer(SFTTrainer):
    """
    Trainer subclass that:
      - Tracks tokens per device during training
      - Reduces tokens globally across FSDP/DDP ranks
      - Computes throughput at end of training
    """
    ...
    def log(self, logs: dict[str, float], start_time: float | None = None) -> None:
        ...
        if self.state.global_step >= 0:
            ...
            if "learning_rate" in logs:
                logs["learning_rate"] = f"{logs['learning_rate']:.5e}"
            # in millions for readability
            # Use "device" instead of "gpu" for generic naming
            device_type = "gpu" if torch.cuda.is_available() else "cpu"
            logs[f"_total_1M_tokens_{device_type}_{rank}"] = (
                f"{(self.total_tokens_this_gpu / 1e6):.3f}"
            )

            # Memory usage (GPU if available, otherwise no memory logging for CPU)
            if torch.cuda.is_available():
                logs["_gpu_mem_allocated_gb"] = (
                    f"{torch.cuda.memory_allocated() / 1e9:.2f}"
                )
        ...
        if self.logged_flops_this_gpus:
            device_type = "GPU" if torch.cuda.is_available() else "device"
            logs[f"TFLOPs/sec/{device_type}"] = (
                f"{self.logged_flops_this_gpus[-1] / 1e12:.2f}"
            )
        if self.logged_mfu_this_gpus and torch.cuda.is_available():
            logs["mfu/GPU"] = f"{self.logged_mfu_this_gpus[-1]:.2f}%"
    ...
    def train(self, *args, **kwargs):
        ...
        # all-reduce token counters
        self._finalize_token_counts()
        if not os.environ.get("DISABLE_MONITORING", "False").lower() == "true":
            self._finalize_flop_counts()
            self._finalize_mfu_counts()
        elapsed = end_time - start_time
        try:
        ...
        if self.is_world_process_zero():
            ...
            print("\n=== TOKEN / THROUGHPUT SUMMARY (PerformanceTrackingTrainer) ===")
            print(
                f"Total tokens per {device_type} (ALL epochs): {self.total_tokens_this_gpu:,}"
            )
            print(f"Total tokens GLOBAL (ALL epochs): {self.total_tokens_global:,}")
            ...
            if elapsed > 0:
                print(f"Total training time (s): {elapsed:.2f}")
                print(
                    f"Tokens/sec per {device_type}: {self.total_tokens_this_gpu / elapsed:,.2f}"
                )
                print(f"Tokens/sec GLOBAL: {self.total_tokens_global / elapsed:,.2f}")
                print(
                    f"Average FLOPs over all {device_type}s: {self.global_average_flops / 1e12:.2f} TFLOPs/sec"
                )
                if torch.cuda.is_available():
                    print(
