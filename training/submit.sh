...
if $DRY_RUN; then
    ...
    mapfile -t configs < <(parallel -k "if ! check_config {}; then echo {}; fi" ::: ${model_names[@]} ::: ${frameworks[@]} ::: ${parallelisms[@]} ::: ${node_configs[@]} ::: ${dataset_names[@]} ::: ${batch_sizes[@]} ::: ${grad_accums[@]} ::: ${enable_compile[@]} ::: ${precisions[@]} ::: ${lrs[@]} ::: ${max_lengths[@]} ::: $(seq $trials))
    ...
    # Pretty Print
    echo Valid Configs:
    echo -e "Model\t\tFramework\t\tParallelism\tNodes\tDataset\tBatch Size\tGrad Accum\tCompile\tPrecision\tLR\tMaxLength\tTrial"
    for config in "${CONFIGS[@]}"; do
        shmonfig=($config)
