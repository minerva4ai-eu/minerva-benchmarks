...
NUMBER_OF_NODES=(4)
TYPE_PARALLELISM=("ddp") # "ddp" "fsdp"
REPEATS=1
MACHINE="cines-adastra-mi250"
MACHINE_TYPE="rocm"
#######################################################
      ...
      for NODES in "${NUMBER_OF_NODES[@]}"; do
        ...
        for GPU_NODE in "${GPU_CONFIGS[@]}"; do
          for parallelism in "${TYPE_PARALLELISM[@]}"; do
            ...
            MAX_MODEL_LENGTHS=($(echo "$CONFIG_JSON" | jq -r '.max_model_length // empty | .[]' 2>/dev/null || echo ""))

            if [ ${#MAX_MODEL_LENGTHS[@]} -eq 0 ]; then
              MAX_MODEL_LENGTHS=(2048)
            fi
