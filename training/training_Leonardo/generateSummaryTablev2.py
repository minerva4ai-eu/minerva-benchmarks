...
BASE_DIR_RESULTS = str(SCRIPT_DIR / "results")
...
OUTPUT_FILE = str(SCRIPT_DIR / f"results/full_benchmark_training_summary_{SUPCOMPUTER_NAME}_{PARTITION_NAME}.csv")
...
def parse_run_path(launch_path: Path):
    ...
    <BASE_DIR_RESULTS>/<framework>/<dataset>/<model>/<Nodes_4-GPUs_16-...>[/launch-N]

    Anchors on BASE_DIR_RESULTS so it works regardless of the directory name.
    """
    ...
    config_str = ""
    try:
        base_parts = Path(BASE_DIR_RESULTS).parts
        if parts[:len(base_parts)] == base_parts:
            rel = parts[len(base_parts):]
            # rel: (framework, dataset, model, config_str[, launch-N])
            if len(rel) > 0:
                framework = rel[0]
            if len(rel) > 1:
                dataset_name = rel[1]
...
def read_and_average_all_json_summaries(launch_dir: Path):
    ...
    """
    json_files = list(launch_dir.rglob("training_summary_*.json"))
    if not json_files:
        return {}
...
def main():
    ...
    # Skip nested ones (a launch-* dir that lives inside another launch-* dir)
    # which arise when CURRENT_DIR points to the launch folder during training.
    config_groups = defaultdict(list)
    for launch_dir in base_results.rglob("launch-*"):
        if not launch_dir.is_dir():
            continue
        if any(part.startswith("launch-") for part in launch_dir.relative_to(base_results).parts[:-1]):
            continue
