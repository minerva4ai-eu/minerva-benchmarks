...
class TokenTrackingTrainer(Trainer):
    ...
    def calculate_mfu(
        ...
        full_step_flops = self.flops_accumulated  # * self.args.logging_steps
        achieved_tflops_per_sec = (full_step_flops / 1e12) / (
    ...
    def log(self, logs: Dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if self.state.global_step > 0:
            ...
            # )
        logs["flops_gpu"] = f"{self.last_logged_flops_this_gpu / 1e12:.2f} TFLOPs"
    ...
    def compute_loss(
        ...
        self.logged_flops_this_gpus.append(int(self.last_logged_flops_this_gpu))
    ...
    def train(self, *args, **kwargs):
        ...
        if self.is_world_process_zero():
            ...
            if elapsed > 0:
                ...
                print(
                    f"Average FLOPs over all GPUs: {self.global_average_flops / 1e12:.2f} TFLOPs"
                )
                print(
                    f"Average FLOPs per GPU: {self.global_average_flops / dist.get_world_size() / 1e12:.2f} TFLOPs"
                )
                print(f"Average MFU across all GPUs: {self.global_average_mfu:.2f}%")
                print(
                    f"Average MFU per GPU: {self.global_average_mfu / dist.get_world_size():.2f}%"
                )
            print("==========================================================\n")
